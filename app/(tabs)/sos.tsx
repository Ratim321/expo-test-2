...
export default function SOSScreen() {
  ...
  const [useRadius, setUseRadius] = useState(false); // Toggle between radius and selection
  ...
  const triggerEmergencyAlert = async () => {
    ...
    try {
      ...
      if (!useRadius && selectedUsers.length > 0) {
      ...
      console.log('SOS create response:', rawResponse);

      if (response.ok) {
        showToast(`Emergency alert sent to ${useRadius ? 'nearby users' : 'selected users'}`, 'success');
        fetchActiveSOSAlerts();
      } else {
  ...
  const confirmSOS = () => {
    if (!useRadius && selectedUsers.length === 0) {
      showToast('Please select at least one user or use radius', 'error');
      return;
    }
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.sosButtonContainer}>
          ...
          <Text style={styles.sosInstructions}>
            {isEmergencyActive 
              ? 'Tap again to cancel' 
              : 'Tap to select users or use radius'}
          </Text>
        </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity
              style={[styles.optionButton, useRadius && styles.optionButtonSelected]}
              onPress={() => {
                setUseRadius(true);
                setSelectedUsers([]);
              }}
...
const styles = StyleSheet.create({
  // Existing styles unchanged up to safetyTipsSection
  container: {
    flex: 1,
